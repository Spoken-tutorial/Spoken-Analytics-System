    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            events_stats.forEach((key, value) => {
                events_table.append("<tr><td>" + key.event_name + "</td><td><div class='progress progress-sm mb-2' style='margin-top: 0.7em;'><div class='progress-bar' role='progressbar' style='width: " + (key.page_views / total_events_page_views).toFixed(2) * 100 + "%' aria-valuenow='44.3' aria-valuemin='0' aria-valuemax='100'></div></div></td><td>" + ((key.page_views / total_events_page_views) * 100).toFixed(2) + "%</td></tr>");
            });
        },
        error: function(err) {
